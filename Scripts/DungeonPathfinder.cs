...
// Dijkstra, DungeonPathfinder.cs
public class DungeonPathfinder {
    private static DungeonPathfinder instance;

    public static DungeonPathfinder GetInstance(Vector2Int size)
    {
        if (instance == null)
        {
            instance = new DungeonPathfinder(size);
        }
        return instance;
    }
    ...
    GridPathFinder<Node> grid;
    SimplePriorityQueue<Node, float> queue;
    HashSet<Node> closed;
    Stack<Vector2Int> stack;

    private DungeonPathfinder(Vector2Int size) {
        grid = new GridPathFinder<Node>(size, Vector2Int.zero);
